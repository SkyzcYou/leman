    ...
    <div class="layui-body">
        ...
        <div class="row">
            <div class="col-md-4 column">
                <a class="btn btn-primary" href="${pageContext.request.contextPath}/toAddComment">新增</a>
            </div>
        </div>
        <table class="table table-hover table-striped">
            ...
            <c:forEach var="comment" items="${requestScope.get('list')}">
                <tr>
                    <td>${comment.getId()}</td>
                    <td>${comment.getEpisode_id()}</td>
                    <td>${comment.getUser_id()}</td>
                    <td>${comment.getContent()}</td>
                    <td>${comment.getCreate_date()}</td>
                    <td>
                        <a href="${pageContext.request.contextPath}/comment/toUpdateComment?id=${comment.getId()}">更改</a> |
                        <a href="${pageContext.request.contextPath}/comment/del/${comment.getId()}">删除</a>
                    </td>
                </tr>
            </c:forEach>
            </tbody>
